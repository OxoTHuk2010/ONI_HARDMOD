...
| Generators | `Generator`, `EnergyGenerator`, `ManualGenerator`, `Building` | Config classes found: `GeneratorConfig`, `WoodGasGeneratorConfig`, `HydrogenGeneratorConfig`, `MethaneGeneratorConfig`, `PetroleumGeneratorConfig`, `PeatGeneratorConfig`, `ManualGeneratorConfig`; runtime points require a later prototype | Research only | Generator wattage/fuel/products remain vanilla in 0.4.3; fuel thermal accounting is not active |
