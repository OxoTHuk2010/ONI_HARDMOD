...
| Generators | `Generator`, `EnergyGenerator`, `ManualGenerator`, `Building`, `Storage`, `PrimaryElement` | Config ID constants confirmed: `ManualGenerator`, `WoodGasGenerator`, `Generator`, `PeatGenerator`, `HydrogenGenerator`, `MethaneGenerator`, `PetroleumGenerator`; runtime points: `Generator.WattageRating`, `StructureTemperaturePayload.OperatingKilowatts`, `StructureTemperaturePayload.TotalEnergyProducedKW`, `EnergyGenerator.EnergySim200ms(float)`, `EnergyGenerator.Emit(...)`, `ManualGenerator.EnergySim200ms(float)`, `Building.EffectDescriptors` | Version 0.4.7 active profile table | Wattage follows the v0.4 matrix; native operating/exhaust heat is suppressed for profiled generators and body heat is injected directly per sim tick; consumed hot fuel adds surplus heat to body/products; direct and stored gas outputs inherit hot fuel temperature; liquid outputs are capped below phase transition |
| Transformers | `PowerTransformer`, `Battery` | `PowerTransformer.ApplyDeltaJoules(float,bool)`, private `battery`, `Battery.ConsumeEnergy(float)`, `Battery.GetDescriptors(GameObject)` | Research only | Active transformer efficiency is inactive in 0.4.7; transformers remain vanilla |
...
| Electrical circuits | `CircuitManager`, `EnergyConsumer` | Type and method research only | Research only | Resistance, voltage-drop, brownouts, and power routing are intentionally inactive in 0.4.7 |
