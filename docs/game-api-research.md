...
| Solar Panel | `SolarPanel`, `Generator`, `StructureTemperatureComponents` | `SolarPanel.EnergySim200ms(float)`, `SolarPanel.CurrentWattage`, `SimMessages.ModifyBuildingEnergy(int,float,float,float)` | Version 0.4.3 | Add 0-5 kDTU/s heat based on actual panel wattage from 0-380 W; no light-source scan |
| Generators | `Generator`, `EnergyGenerator`, `ManualGenerator`, `Building` | Config ID constants confirmed: `ManualGenerator`, `WoodGasGenerator`, `Generator`, `PeatGenerator`, `HydrogenGenerator`, `MethaneGenerator`, `PetroleumGenerator`; runtime points: `Generator.WattageRating`, `StructureTemperaturePayload.OperatingKilowatts`, `Building.EffectDescriptors` | Version 0.4.4 active profile table | Wattage and body heat follow the v0.4 matrix for profiled fuel generators; fuel/products remain vanilla and fuel thermal accounting is not active |
