...
For v0.7 asteroid-size testing, create a new world and choose one of the generated `Hardcore ... Half`, `Hardcore ... Quarter`, or `Hardcore ... Eighth` clusters. These presets are separate worldgen assets; they do not shrink existing saves or replace vanilla clusters. `Quarter` targets about 25% map area. `Eighth` is a more aggressive experimental target around 12.5% map area. Compact presets use compact PowerTree defaults, compact subworld variants, and compact layout rules so starter, lava/core, surface/regolith, and resource biomes do not reserve full-size vanilla regions. Compact presets pin actual space to the top surface tag, keep surface/regolith crust directly under it, use a thinner `WorldBorderThickness: 3`, force compact subworld borders to `0..2`, and add several optional geyser/vent placement attempts while avoiding mandatory warp, Gravitas, story, and large POI template guarantees. DLC compact worlds also add a shared compact DLC biome pool so vanilla-style Spaced Out starts can use swamp, rust, radioactive, wasteland, frozen, ocean, marsh, oil, and barren DLC subworlds when space exists. Half remains a larger, less aggressive preset, but its Spaced Out vanilla-style worlds also use the shared DLC pool, compact only the starting subworld, and replace hard template guarantees with optional resource attempts so vanilla biomes stay present without consuming most of the map.
